...
export default {
    ...
    methods: {
        ...
        onDelete(pid, rid, numeros) {
            this.$swal.fire({
                title: 'Liberar numero(s): '+numeros,
                showDenyButton: true,
                ...
                if (result.isConfirmed) {
                    this.loading = true;
                    axios.get(route('raffles.raffleRemoveReserve', {
                        participant: pid,
                        raffle: rid
                    }))
                        .then(response => {
    ...
    mounted() {
         //console.log(this.data, this.results);
    }
}
</script>
<template>
    ...
    <div v-if="results?.data && results?.data?.length > 0" class="flex flex-row flex-wrap justify-center">
        <div v-for="(participant, index) in results?.data?.participants?.reserved?.ranking" :key="index"
    ...
    <div class="flex flex-row flex-wrap w-full py-2 mt-4 rounded-lg bg-base-100">
        ...
        <div class="flex-row items-center hidden w-full py-2 m-2 rounded-lg lg:flex bg-base-200">
            ...
            <div class="flex justify-center w-1/12 text-neutral/70">Ações</div>
        </div>
        <div v-if="results?.data && results?.data?.length > 0" class="w-full pr-3">
            ...
            <div v-for="(sale, index) in results?.data" :key="sale.id" @click="handleAccordion(index)"
                ...
                <div class="flex w-full px-2 mb-1 break-all lg:mb-0 lg:justify-center text-neutral/70 lg:w-1/12">
                    <TicketIcon class="flex w-6 mr-1 lg:m-0 lg:hidden text-neutral/70" />{{ sale?.reserved }}
                </div>
                ...
                <div>
                    <button type="button" @click="onDelete(sale?.id, sale?.raffle_id, sale?.numbers)" class="text-white btn btn-error btn-block btn-sm lg:btn-md">
                        <MinusCircleIcon class="w-6" /> Liberar numeros
                    </button>
                </div>
            ...
            <div class="flex flex-row w-full px-2"
                :class='{ "hidden": results?.data?.participants?.reserved?.data?.last_page == 1 }'>
        ...
        <div v-else class="flex flex-row flex-wrap items-center justify-center w-full py-8 mb-4 rounded-lg bg-base-100">
            <span class="text-base text-xl font-medium title-font text-neutral/70">Nenhuma reserva encontrada</span>
        </div>
    </div>
</template>
